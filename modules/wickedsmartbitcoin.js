async function wickedsmartbitcoin() {
  if (Math.random() < 0.4) {
    ...
    const msg =
      `Bitcoin Price On ${getdate}\n\n` + `https://wickedsmartbitcoin.com/final_frames/price_on_this_day.png\n\n` + `#bitcoin #priceonthisday #pricehistory`;

    return msg;
  }

  if (Math.random() < 0.5) {
    const msg = `Bitcoin Node Count Over Time\n\n` + `https://wickedsmartbitcoin.com/final_frames/node_count.png\n\n` + `#bitcoin #nodecount #decentralization`;

    return msg;
  }

  if (Math.random() < 0.5) {
    const msg = `Bitcoin Halving Progress\n\n` + `https://wickedsmartbitcoin.com/final_frames/halving_progress.png\n\n` + `#bitcoin #halving #btc`;
